          ...
          if (port) {
            ...
            if (elementView) {
              ...
              if (portElement && portElement.length) {
                ...
                target.selector = elementView.getSelector(portElement[0] as SVGElement);
                link.target(target);
              }
            }
          }
          ...
          if (port) {
            ...
            if (elementView) {
              ...
              if (portElement && portElement.length) {
                ...
                source.selector = elementView.getSelector(portElement[0] as any);
                link.source(source);
              }
            }
          }
    ...
    if (portElement) {
      ...
      if (newTargetPort) {
        ...
        if (portElement.classList.contains('flo-input-multiport')) {
          ...
          const channels = metadata instanceof AppMetadata ? (metadata as AppMetadata).inputChannels : undefined;
    ...
    if (portElement) {
      ...
      if (newSourcePort) {
        ...
        if (portElement.classList.contains('flo-output-multiport')) {
          ...
          const channels = metadata instanceof AppMetadata ? (metadata as AppMetadata).outputChannels : undefined;
          if (Array.isArray(channels) && channels.length > 0) {
            const availableChannels = [...channels];
